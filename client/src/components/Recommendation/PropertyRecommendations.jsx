...
const PropertyRecommendations = () => {
  ...
  const fetchPreferences = async () => {
    ...
    try {
      ...
      if (response.status === 404) {
        toast.error(
          "Please set your preferences before requesting recommendations."
        );
        setPreferences(null);
        setLoading(false);
        return;
  ...
  const fetchRecommendations = async () => {
    try {
      const response = await fetch(
        "http://localhost:3000/recommendedProperties/recommendations",
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    ...
    if (preferences && isLoggedIn) fetchRecommendations();
    if (!isLoggedIn) {
      setLoading(false);
    }
  ...
  if (!isLoggedIn) {
    return (
      <div className="login-prompt">
        <p>
          Please{" "}
          <a className="update-preference" href="/login">
            login
          </a>{" "}
          to view personalized property recommendations.
        </p>
      </div>
    );
  }
  ...
  if (!preferences) {
    return (
      <div>
        ...
        <a className="update-preference" href="/account-settings/preferences">
          Update Preferences
        </a>
      </div>
    );
