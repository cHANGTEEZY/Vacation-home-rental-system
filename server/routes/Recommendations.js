    ...
    return await Promise.all(
      imageUrls.map(async (imageKey) => {
        ...
        return getSignedUrl(s3, command, {
          expiresIn: SCORE_WEIGHTS.URL_EXPIRY,
        });
      })
    );
  } catch (error) {
...
const calculateContentScore = (property, preferences) => {
  ...
  const priceScore = Math.max(
    0,
    1 - Math.abs(price - prefPrice) / SCORE_WEIGHTS.PRICE_THRESHOLD
  );
  const regionScore =
    property.property_region === preferences.prefered_property_region ? 1 : 0;
  const typeScore =
    property.property_type === preferences.prefered_property_type ? 1 : 0;
...
const calculateHybridScore = (
  ...
  return Number(
    (
      normalizedContentScore * contentWeight +
      normalizedCollabScore * adjustedHistoryWeight
    ).toFixed(2)
  );
};
...
router.post("/recommendations", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const properties = await getUnviewedProperties(
      userId,
      SCORE_WEIGHTS.MAX_RECOMMENDATIONS
    );
